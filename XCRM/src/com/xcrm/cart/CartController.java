...
public class CartController extends AbstractController {
  ...
  public void index() {
    ...
    List<Productpic> productpics = Productpic.dao.find( "select * from productpic where productid=?", this.getPara( "pid" ) );
  ...
  private List<Tuple<AttributeID, List<String>>> getAttrs() {
    List<Attributevalue> list = Attributevalue.dao.find( "select * from attributevalue where category=1 and objectid=?", this.getPara( "pid" ) );
    ...
    for ( Attributevalue attributevalue : list ) {
      ...
      if ( !StringUtils.isEmpty( attrValue ) ) {
        attrs.add( new Tuple<AttributeID, List<String>>( AttributeID.getById( attributevalue.getAttributeid() ), Arrays.asList( attrValue.split( Constant.COMMA ) ) ) );
      }
  ...
  private Map<String, Priceinventoryvalue> getPrice() {
    List<Priceinventoryvalue> list = Priceinventoryvalue.dao.find( "select pi.* from priceinventoryvalue pi left join price pr on pi.priceid=pr.id  where pr.product=?",
        this.getPara( "pid" ) );
  ...
  public void save() {
    ...
    if ( !editorderflag.isEmpty() && editorderflag.contains( "editorder" ) ) {
      ...
      Bookitem bookitem = this.getModel( Bookitem.class, "", true );
      bookitem.setUser( getCurrentUserId() );
      bookitem.setStatus( Bookitem.STATUS_INIT );
      bookitem.setDate( new Date() );
      if(bookitem.getDiscount() == null){
        bookitem.setDiscount( 100 );
      }
