...
public class ImgUtil {
	public static void createThumb(String filename, String thumbfilename) {
		try {
			File fi = new File(filename); // 大图文件
			File fo = new File(thumbfilename); // 将要转换出的小图文件
			int nw = 220;
			/*
	...
	private static String getThumbFilename(String filename) {
		if (StringUtils.isEmpty(filename))
			throw new RuntimeException("getThumbFilename params filename is null");
		return filename + ".thumb";
	}

	public static String getThumb(String filename) {
		String thumbfilename = getThumbFilename(filename);
		if (!(new File(thumbfilename).exists())) {
			createThumb(filename, thumbfilename);
		}
